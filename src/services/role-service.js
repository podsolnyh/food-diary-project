...
class RoleService {
    static async createRole(roleObject) {
        const roleWithSameName = await RoleRepository.readRoleByName(
            roleObject.name,
        );
        if (roleWithSameName) {
    ...
    static async getListOfRoles() {
        return RoleRepository.readListOfRoles();
    }

    static async getRoleByID(roleID) {
        const role = await RoleRepository.readRoleByID(roleID);
        if (!role) {
    ...
    static async getRoleByName(roleName) {
        const role = await RoleRepository.readRoleByName(roleName);
        if (!role) {
            throw new NotFoundException('Role is not found');
        }
        return role;
    ...
    static async editRoleByID(roleID, roleObject) {
        ...
        return RoleRepository.updateRoleByID(roleID, roleObject);
    }
